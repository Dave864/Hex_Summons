class_name MapTile
extends Spatial
# Represents an individual tile of a battle map


# Indicates that this tile has been selected
...
# -x   |  -y
#     +z
var _cube_coord: Vector3 setget set_cube_coord, get_cube_coord


# Called when the node enters the scene tree for the first time
func _ready():
	set_index(-1)
	set_cube_coord(Vector3.ZERO)
...
# Indicates that the tile has been selected, passing on the tile's index
func _on_TileSelector_selected():
	emit_signal("selected", _index)
