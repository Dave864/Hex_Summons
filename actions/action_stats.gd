...
func check_for_required_resources() -> void:
	...
	assert(
			...
			"Error: ActionStats area_range is neither a CardinalArea "
			+ "or RingArea resource."
	)
	if dead_range != null:
		assert(
				dead_range is CardinalArea or dead_range is RingArea,
				"Error: ActionStats dead_range is neither a CardinalArea "
				+ "or RingArea resource."
		)
	assert(
			effect_range != null,
	...
	assert(
			...
			"Error: Action %s effect_range is not an AreaRange resource."
	)
